...
public class WriteDietActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        tvDietWriteDate = findViewById(R.id.tv_diet_write_writedate);
        editDietWeight = findViewById(R.id.edit_diet_write_weight);
        editDietMemo = findViewById(R.id.edit_diet_write_memo);
        btnDietKcalTable = findViewById(R.id.btn_diet_kcal_table);
        btnDietSave = findViewById(R.id.btn_diet_write_save);

        String pkg = getPackageName();
        int tmpID;
        for (int i=0; i<3; i++) {
            tmpID = getResources().getIdentifier("edit_diet_write_menu"+i, "id", pkg);
            editDietMenus[i] = findViewById(tmpID);
            tmpID = getResources().getIdentifier("edit_diet_write_kcal"+i, "id", pkg);
        ...
        btnDietSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(result > 0) {
                    Intent responseIntent = new Intent(WriteDietActivity.this, ShowDietActivity.class);
                    responseIntent.putExtra("writeDate", selectedDate);

                    startActivity(responseIntent);
                } else {
                    Toast.makeText(WriteDietActivity.this, "에러가 발생했습니다. 다시 시도해주세요.", Toast.LENGTH_SHORT).show();
                }
            }
        });
